pacman::p_load(sp, geobr)

brasil = read_country(year = 2018)

brasil <- brasil %>%
  st_cast("POLYGON") %>%
  mutate(area = st_area(.)) %>%
  group_by(1) %>%          # identifier for each feature
  filter(area == max(area)) %>%         # keep only the biggest polygon
  ungroup() %>%
  select(-area) %>%
  st_as_sf()

poligono = sp::Polygon(sf::st_coordinates(brasil$geom)[, 1:2])
poligonos = sp::Polygons(list(poligono), ID = "1")

spatial_poligono = SpatialPolygons(list(poligonos))

#spatial_df = SpatialPolygonsDataFrame(Sr = spatial_poligono, data = dados |> select(-lat, -lon))

proj4string(spatial_poligono) = CRS("+init=epsg:4674")

grade <- makegrid(spatial_poligono, n = 10000)
colnames(grade) <- c("x", "y")

grade_pts <- SpatialPoints(
  coords      = grade, 
  proj4string = CRS(proj4string(spatial_poligono))
)

grade_pts_in <- grade_pts[spatial_poligono, ]

plot(grade_pts_in)

saveRDS(grade_pts_in, "grade.rds")